...
public enum Urls {
	...
	WARS_AA_LIMIT(BASE.getFormat() + "wars/%d&key=%s&alliance_id=%s"), // UNTESTED
	BANK(BASE.getFormat() + "alliance-bank/?allianceid=%d&key=%s"), // UNTESTED
	ALLIANCE_MEMBERS(BASE.getFormat() + "alliance-members/?allianceid=%d&key=%s"), // UNTESTED
	APPLICANTS(BASE.getFormat() + "applicants/%d/?key=%s"), // UNTESTED
	...
	TRADE_HISTORY_RESOURCES_AND_RECORDS(BASE.getFormat() + "trade-history/key=%s&records=%d&resources=%s"), // UNTESTED
	...
	WAR_ATTACKS_MAX(BASE.getFormat() + "war-attacks/key=%s&max_war_id=%d"), // UNTESTED
	WAR_ATTACKS_RANGE(BASE.getFormat() + "war-attacks/key=%s&max_war_id=%d&min_war_id=%d"); // UNTESTED
